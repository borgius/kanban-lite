...
### Changed
- **Expanded board background library**: Added five more colorful `fancy` board background presets (`nebula`, `lagoon`, `candy`, `ember`, `violet`) on top of the existing options, so the settings panel now offers a broader set of decorative ambient canvases alongside the simpler plain presets.
