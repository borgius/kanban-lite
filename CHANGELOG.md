...
- **Drop wasted seed-config read on runtime-hosted `readConfig`**: `readConfigRepositoryDocument` previously called `readFileConfigRepositoryDocument` (an `fs.readFileSync` + `JSON.parse`) even when the runtime host had already returned an authoritative config and no seed fallback was requested. The result was never used in that branch. The seed read is now deferred so it only runs when it can actually affect the outcome (no runtime-host result, or an explicit `allowSeedFallbackOnProviderError` retry after a host error). This removes one synchronous fs read + JSON parse from every Cloudflare Worker request and every standalone request once a runtime host is installed. No behavior change.

- **All HTTP / WS / MCP requests share one config read**: The standalone route dispatcher (`dispatch.ts`) and the Cloudflare Worker MCP handler (`worker/mcp-handler.ts`) now wrap every request in `withConfigReadCache`. Previously only `syncWebviewMessages` and WS broadcasts were scoped, so regular REST, mobile, and MCP-over-HTTP requests performed N `readConfig` provider round-trips per request (each `getSettings`, `listColumns`, `listBoards`, `getLabels`, `getMinimizedColumns`, plus per-handler default-board lookups). On remote `config.storage` backends (Cloudflare KV, MySQL, PostgreSQL), this collapses those N round-trips into one per request. The plugin-handler collection pass in the dispatcher also dropped from two iterations to one, removing the redundant `registrationOptions` rebuild on dispatcher creation.
...
- **Elevated `plugin-settings.auth.*` permissions for auth-governing plugin options**: Changing plugin options and selecting providers is already available across API (`PUT /api/plugin-settings/:capability/:providerId/options` and `/select`), CLI (`plugin-settings update-options|select`), and MCP (`update_plugin_settings_options`, `select_plugin_settings_provider`). Mutating the auth-governing capabilities (`auth.identity`, `auth.policy`, `auth.visibility`) — which control who can sign in and what each role may do — now requires the new elevated `plugin-settings.auth.update` action in addition to `plugin-settings.update`, and reading a single auth-governing provider requires `plugin-settings.auth.read` in addition to `plugin-settings.read`. This lets an integration (for example an MCP agent) be granted the power to change non-auth plugin options and other settings without also gaining the ability to rewrite authentication/authorization config, closing a privilege-escalation path. The shipped RBAC `admin` role receives all four actions by default; `manager` and `user` receive none. Enforced once in the SDK (`getPluginSettings`, `selectPluginSettingsProvider`, `updatePluginSettingsOptions`) so API, CLI, and MCP stay in parity.

- **Cloudflare Access identity provider**: `kl-plugin-cloudflare` now advertises `auth.identity: cloudflare`, validates Cloudflare Access JWT assertions with RS256 JWKS verification plus exact issuer/audience/lifetime checks, exposes schema-driven no-secret options for Access setup and role mapping, and lets the Worker pass `CF-Access-Jwt-Assertion` into the existing bearer auth context when `Authorization` is absent.
