...
- **OpenAPI is now the public HTTP contract source of truth**: The standalone server now derives built-in route registration and OpenAPI path metadata from a shared contract registry, plugin-owned HTTP surfaces such as local mobile auth and webhooks contribute their own OpenAPI fragments from the owning package, and the checked-in remote path types plus shared request helper now drive `RemoteKanbanSDK`, the Vercel AI adapter, and the n8n remote transport. This closes several long-standing route/method/body drift issues such as task move/checklist/comment endpoints while keeping `KanbanSDK` as the behavior source of truth behind those public HTTP contracts.
