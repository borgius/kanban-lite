...
Card open/preview flows over that HTTP bridge now persist workspace active-card selection through a Cloudflare Durable Object generated by `scripts/deploy-cloudflare-worker.mjs`, so `/api/tasks/active`, preview routes, and follow-up requests keep working even though the Worker runtime has no writable local filesystem. Non-Worker hosts still fall back to the local `.active-card.json` sidecar.
