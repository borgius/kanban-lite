...
When `config.storage` is backed by the Cloudflare D1 bridge, Worker refreshes also merge runtime-created boards already persisted in D1 with boards that arrive in a newly deployed bootstrap config. That keeps API-created boards visible across redeploys while still surfacing newly added boards from the latest shipped `.kanban.json`.
