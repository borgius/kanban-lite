...
- `kl-plugin-auth` package with three fully-shipped provider ids: `noop`, `rbac`, and `local`,
...
- switches both auth capability ids to the `rbac` provider pair supplied by `kl-plugin-auth`
