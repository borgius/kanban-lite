 ...
 */
export type SDKBeforeEventType = 'card.create' | 'card.update' | 'card.move' | 'card.delete' | 'card.transfer' | 'card.action.trigger' | 'card.purgeDeleted' | 'comment.create' | 'comment.update' | 'comment.delete' | 'column.create' | 'column.update' | 'column.delete' | 'column.reorder' | 'column.setMinimized' | 'column.cleanup' | 'attachment.add' | 'attachment.remove' | 'settings.update' | 'board.create' | 'board.update' | 'board.delete' | 'board.action.config.add' | 'board.action.config.remove' | 'board.action.trigger' | 'board.setDefault' | 'log.add' | 'log.clear' | 'board.log.add' | 'board.log.clear' | 'storage.migrate' | 'label.set' | 'label.rename' | 'label.delete' | 'webhook.create' | 'webhook.update' | 'webhook.delete' | 'form.submit';
/**
...
export interface AfterEventPayload<TResult = unknown> {
     ...
     * into the last checkpoint so the lifecycle budget is `1 + total handler attempts`.
     */
    readonly meta?: Record<string, unknown>;
}
/**
 ...
 */
export declare function sanitizeCard(card: Card): Omit<Card, 'filePath'>;
/**
 ...
 */
export type AuthErrorCategory = 'auth.identity.missing' | 'auth.identity.invalid' | 'auth.identity.expired' | 'auth.policy.denied' | 'auth.policy.unknown' | 'auth.provider.error';
/**
...
export declare const ERR_CARD_STATE_IDENTITY_UNAVAILABLE = "ERR_CARD_STATE_IDENTITY_UNAVAILABLE";
/** Stable machine-readable error for card-state calls when no provider is active. */
export declare const ERR_CARD_STATE_UNAVAILABLE = "ERR_CARD_STATE_UNAVAILABLE";
/** Public card-state error codes shared across SDK, API, CLI, and MCP hosts. */
export type CardStateErrorCode = typeof ERR_CARD_STATE_IDENTITY_UNAVAILABLE | typeof ERR_CARD_STATE_UNAVAILABLE;
/** Stable mode name for the auth-absent card-state default actor contract. */
export declare const CARD_STATE_DEFAULT_ACTOR_MODE = "auth-absent-only";
/**
...
export declare const DEFAULT_CARD_STATE_ACTOR: Readonly<{
    ...
    readonly mode: "auth-absent-only";
}>;
...
export declare class CardStateError extends Error {
    ...
    readonly code: CardStateErrorCode;
    /** Status classification derived from {@link code}. */
    readonly availability: Exclude<CardStateAvailability, 'available'>;
...
export interface KanbanCliPlugin {
     ...
     */
    run(subArgs: string[], flags: Record<string, string | boolean | string[]>, context: CliPluginContext): Promise<void>;
}
/**
