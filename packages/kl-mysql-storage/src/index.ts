  ...
  close(): void {
    if (this._pool) {
      ...
      pool.end().catch((err: unknown) => {
        console.error('[kl-mysql-storage] pool.end() error:', err)
      })
    }
  }
...
export function createMysqlAttachmentPlugin(engine: StorageEngine): AttachmentStoragePlugin {
  if (engine.type !== 'mysql') {
    throw new Error(
      'kl-mysql-storage: attachment plugin requires an active mysql card.storage engine.'
    )
  }
 ...
 *
 * Provider id: `mysql`
 * Install: `npm install kl-mysql-storage mysql2`
 *
 * @example `.kanban.json`
 * ```json
 * {
...
export const cardStoragePlugin: CardStoragePlugin = {
  ...
  createEngine(kanbanDir: string, options?: Record<string, unknown>): MysqlStorageEngine {
    ...
    if (typeof database !== 'string' || !database) {
      throw new Error(
        'kl-mysql-storage: MySQL storage requires a "database" option. ' +
...
export const attachmentStoragePlugin: AttachmentStoragePlugin = {
  ...
  async copyAttachment(sourcePath: string, card: Card): Promise<void> {
    ...
    if (!kanbanDir) {
      throw new Error(
        'kl-mysql-storage: attachmentStoragePlugin.copyAttachment requires KL_KANBAN_DIR env var ' +
