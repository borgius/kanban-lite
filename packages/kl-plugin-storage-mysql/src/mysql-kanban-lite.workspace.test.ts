/**
 * Workspace integration tests for kl-plugin-storage-mysql.
 *
 * Proves that kl-plugin-storage-mysql is resolvable via the kanban-lite KanbanSDK in
...
describe('kl-plugin-storage-mysql: consumption via kanban-lite workspace SDK', () => {
  ...
  it('KanbanSDK reads kl-plugin-storage-mysql from .kanban.json plugins config', () => {
    fs.writeFileSync(
