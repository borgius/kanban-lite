...
afterEach(() => {
  __setPostgresqlConfigStorageRunnerForTests(null)
})

describe('kl-plugin-storage-postgresql config.storage provider', () => {
  it('advertises config.storage in the package manifest', () => {
    expect(pluginManifest.capabilities['config.storage']).toEqual(['postgresql'])
  })

  it('round-trips workspace config with the same connection options payload used by card.storage', () => {
    const commands: PostgresqlConfigStorageCommand[] = []
    let storedDocument: Record<string, unknown> | null = null

    __setPostgresqlConfigStorageRunnerForTests((command) => {
      commands.push(structuredClone(command) as PostgresqlConfigStorageCommand)
