...
export const attachmentStoragePlugin: AttachmentStoragePlugin = {
  ...
  getCardDir(card: Card): string | null {
    ...
    // real per-request attachment delegation goes through createPostgresqlAttachmentPlugin.
    void card
    return null
  },
  async copyAttachment(sourcePath: string, card: Card): Promise<void> {
    // Direct invocation without an engine reference is only used in contract
    // validation; real usage goes through createPostgresqlAttachmentPlugin.
    const kanbanDir = process.env['KL_KANBAN_DIR']
    if (!kanbanDir) {
      throw new Error(
        'kl-plugin-storage-postgresql: attachmentStoragePlugin.copyAttachment requires KL_KANBAN_DIR env var '
        + 'or an engine instance via createPostgresqlAttachmentPlugin(engine).'
      )
    }
