...
const PROVIDER_ID = 'kl-s3-attachment-storage'
...
function readEnvConfig(): { bucket: string; prefix: string } {
  ...
  if (!bucket) {
    throw new Error(
      '[kl-s3-attachment-storage] KL_S3_BUCKET environment variable is required. ' +
...
export const attachmentStoragePlugin: AttachmentStoragePlugin = {
  ...
  async copyAttachment(sourcePath: string, card: Card): Promise<void> {
    ...
    if (!safeFilename) {
      throw new Error(
        `[kl-s3-attachment-storage] Unsafe attachment filename: "${filename}". ` +
