...
function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'kl-sqlite-card-state-ws-'))
}
...
describe('kl-sqlite-card-state via workspace KanbanSDK', () => {
  afterEach(() => {
  ...
  it('is discoverable as a workspace package with the expected manifest name', () => {
    const workspaceRoot = findWorkspaceRoot(__dirname)
    const packageJsonPath = path.join(workspaceRoot, 'packages', 'kl-sqlite-card-state', 'package.json')
    const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf-8')) as { name?: string }

    expect(packageJson.name).toBe('kl-sqlite-card-state')
  })
