/**
 * Workspace integration tests for kl-sqlite-storage.
 *
 * Proves that kl-sqlite-storage is resolvable and functional when consumed
 ...
 *   KanbanSDK → resolveCapabilityBag → loadExternalModule → packages/kl-sqlite-storage
 *
...
describe('kl-sqlite-storage: consumption via kanban-lite workspace SDK', () => {
  ...
  it('KanbanSDK resolves kl-sqlite-storage via the "sqlite" provider alias', () => {
  ...
  it('KanbanSDK reads kl-sqlite-storage from .kanban.json storageEngine config', () => {
    fs.writeFileSync(
