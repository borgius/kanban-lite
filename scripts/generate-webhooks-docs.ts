...
function generate(): string {
  ...
  lines.push('- Current releases retain a built-in webhook fallback while `kl-webhooks-plugin` is absent, so CRUD and delivery remain backward-compatible during the rollout.')
  lines.push('')
  ...
  lines.push('')
  lines.push('Webhook delivery can be selected explicitly through `webhookPlugin`, while webhook registrations themselves stay in the existing top-level `.kanban.json` `webhooks` array:')
  lines.push('')
  lines.push('```json')
  lines.push('{')
  lines.push('  "webhookPlugin": {')
  lines.push('    "webhook.delivery": {')
  lines.push('      "provider": "webhooks"')
  lines.push('    }')
  lines.push('  }')
  lines.push('}')
  lines.push('```')
  lines.push('')
  lines.push('Registered webhooks are stored in your project\'s `.kanban.json` file:')
  lines.push('')
  lines.push('```json')
  lines.push('{')
  lines.push('  "webhooks": [')
  ...
  lines.push('')
  lines.push('Tools: `list_webhooks`, `add_webhook`, `update_webhook`, `remove_webhook`')
  lines.push('')
