...
function generate(): string {
  ...
  lines.push('# Webhooks')
  lines.push('')
  lines.push('Kanban Lite webhook delivery is owned by `kl-plugin-webhook`. It delivers committed SDK after-events via HTTP POST to any registered endpoint.')
  lines.push('')
  lines.push('## Overview')
  lines.push('')
  lines.push('- Webhooks fire from **all interfaces**: REST API, CLI, MCP server, and the UI (via the standalone server).')
  lines.push('- `kl-plugin-webhook` owns runtime delivery, webhook registry CRUD, the standalone `/api/webhooks` routes, the `kl webhooks` CLI family, and webhook MCP tool registration where those plugin seams exist.')
  ...
  lines.push('- The default runtime provider id is `webhooks`, which resolves to the external `kl-plugin-webhook` package.')
  lines.push('- Advanced SDK consumers can use `sdk.getExtension(\'kl-plugin-webhook\')`; the direct webhook SDK methods remain stable compatibility shims.')
  ...
  lines.push('')
  lines.push('Install `kl-plugin-webhook` in the same environment that loads Kanban Lite:')
  lines.push('')
  lines.push('```bash')
  lines.push('npm install kl-plugin-webhook')
  lines.push('```')
  lines.push('')
  lines.push('For local development, a sibling checkout at `../kl-plugin-webhook` is resolved automatically. `npm link ../kl-plugin-webhook` is optional when you want an explicit local package link.')
  lines.push('')
  ...
  lines.push('      "provider": "kl-plugin-webhook",')
  lines.push('      "options": {')
  lines.push('        "webhooks": [')
  lines.push('          {')
  ...
  lines.push('            "headers": [')
  lines.push('              { "name": "Authorization", "value": "' + 'Bearer ' + '$' + '{KL_WEBHOOK_TOKEN}" },')
  lines.push('              { "name": "X-Source", "value": "kanban-lite" }')
  lines.push('            ],')
  lines.push('            "transform": "{ text: (.event + \\" on \\" + .data.title) }",')
  ...
  lines.push('')
  lines.push('For plugin-aware consumers, `kl-plugin-webhook` also contributes an additive SDK extension bag available through `sdk.getExtension(\'kl-plugin-webhook\')`. Those extension methods and the direct SDK methods share the same backing store; the direct methods remain the compatibility path for existing callers.')
  lines.push('')
  lines.push('### REST API')
  lines.push('')
  lines.push('These routes are plugin-owned when `kl-plugin-webhook` is loaded by the standalone host.')
  lines.push('')
  ...
  lines.push('| `POST` | `/api/webhooks/transform/test` | Evaluate a jq `transform` against a sample (or supplied) payload before saving |')
  lines.push('')
  lines.push('### CLI')
  lines.push('')
  lines.push('These commands are plugin-owned when `kl-plugin-webhook` is loaded by the CLI host.')
  lines.push('')
  lines.push('```bash')
  ...
  lines.push('')
  lines.push('# Register a webhook with custom headers and a jq payload transform')
  lines.push('kl webhooks add --url https://example.com/hook --events "task.*" \\')
  lines.push('  --header "Authorization: ' + 'Bearer ' + '$' + '{KL_WEBHOOK_TOKEN}" \\')
  lines.push('  --header "X-Source: kanban-lite" \\')
  lines.push('  --transform \'{ text: .event, title: .data.title }\'')
  lines.push('')
  ...
  lines.push('')
  lines.push('These tools are plugin-owned when `kl-plugin-webhook` is loaded by the MCP host through the narrow `mcpPlugin.registerTools(...)` seam. Public tool names, schemas, auth wrapping, and secret redaction behavior remain unchanged: `list_webhooks`, `add_webhook`, `update_webhook`, `remove_webhook`. The `add_webhook` and `update_webhook` tools accept optional `headers` (array of `{ name, value }`, supporting `' + '$' + '{ENV_VAR}` injection) and `transform` (jq expression) parameters.')
  lines.push('')
  ...
  lines.push('```')
  lines.push('')
  lines.push('Reserved headers (`Content-Type`, `Content-Length`, and the signature/event headers) are always computed by the delivery engine and cannot be overridden.')
  lines.push('')

  // jq transform
  lines.push('## Payload transform (jq)')
  lines.push('')
  lines.push('A webhook may define an optional `transform` jq expression. When set, it is applied to the JSON delivery envelope before the request is sent, so you can reshape the payload to match a third-party API (for example Slack or Microsoft Teams) without an intermediary service.')
  lines.push('')
  lines.push('- jq runs through [`jq-web`](https://github.com/stainless-api/jq-web), a WebAssembly build of jq that works in Node.js, the browser, and Cloudflare Workers.')
  lines.push('- The transform input is the full envelope (`event`, `timestamp`, `actor`, `boardId`, `meta`, `data`); the result becomes the new request body.')
  lines.push('- The HMAC signature and `Content-Length` are computed over the **transformed** body.')
  lines.push('- If the expression is invalid or fails at runtime, delivery falls back to the original payload and logs the error — a bad transform never silently drops a delivery.')
  lines.push('')
  lines.push('```jq')
  lines.push('{ text: (.event + " on " + .data.title) }')
  lines.push('```')
  lines.push('')
  lines.push('### Testing a transform before saving')
  lines.push('')
  lines.push('The standalone server exposes `POST /api/webhooks/transform/test` so you can validate an expression against a representative payload before saving. The Plugin Options editor in the UI uses this endpoint to provide an inline jq tester with an editable sample payload.')
  lines.push('')
  lines.push('Request body: `{ "transform": "<jq>", "payload": <optional JSON> }`. When `payload` is omitted, a built-in sample `task.created` envelope is used. The response is `{ "ok": true, "data": { "result": <transformed> } }`, or `{ "ok": false, "error": "<message>" }` (HTTP `400` for an empty expression, `422` for an invalid expression).')
  lines.push('')
